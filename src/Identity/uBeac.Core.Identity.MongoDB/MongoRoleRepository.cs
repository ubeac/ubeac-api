...
{
    public MongoRoleRepository(TContext mongoDbContext, IApplicationContext appContext, IEntityHistoryRepository<TRoleKey, TRole> historyRepository) : base(mongoDbContext, appContext, historyRepository)
    {
        // Create Indexes
        try
        {
...
public class MongoRoleRepository<TRole, TContext> : MongoRoleRepository<Guid, TRole, TContext>, IRoleRepository<TRole>
    ...
    where TContext : IMongoDBContext
{
    public MongoRoleRepository(TContext mongoDbContext, IApplicationContext appContext, IEntityHistoryRepository<TRole> historyRepository) : base(mongoDbContext, appContext, historyRepository)
    {
    }
}
