...
{
    public MongoUnitRepository(TContext mongoDbContext, IApplicationContext appContext, IEntityHistoryRepository<TKey, TUnit> historyRepository) : base(mongoDbContext, appContext, historyRepository)
    {
        // Create Indexes
        try
        {
...
public class MongoUnitRepository<TUnit, TContext> : MongoUnitRepository<Guid, TUnit, TContext>, IUnitRepository<TUnit>
    ...
    where TContext : IMongoDBContext
{
    public MongoUnitRepository(TContext mongoDbContext, IApplicationContext appContext, IEntityHistoryRepository<TUnit> historyRepository) : base(mongoDbContext, appContext, historyRepository)
    {
    }
}
